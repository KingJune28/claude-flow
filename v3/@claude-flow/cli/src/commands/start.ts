...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
    output.printError('KynjalFlow is not initialized in this directory');
  ...
  output.writeln();
  output.writeln(output.bold('Starting KynjalFlow'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
    output.printSuccess('KynjalFlow is running!');
    output.writeln();
...
const stopCommand: Command = {
  name: 'stop',
  description: 'Stop the KynjalFlow system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Stopping KynjalFlow'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
        message: 'Are you sure you want to stop KynjalFlow?',
        default: false
      });
...
const restartCommand: Command = {
  name: 'restart',
  description: 'Restart the KynjalFlow system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Restarting KynjalFlow'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the KynjalFlow orchestration system',
